...
% Comments: This program, in addition to printing the solution to stdout,
%           attempts to simulate the sounds of the telephone keypresses
%           corresponding to the output. The program produces a PDF file
%           containing sheet music as well as a MIDI file for your
%           listening pleasure.

\version "2.12.2"

\header{
   title = "T9 Spelling: An Epic Composition"
   instrument = "For the Telephone"
   composer = "KirarinSnow"
   date = "March 2010"
}

out = ""

total = #(read) #(read-char) % get rid of newline after case number
...
process = #(define-music-function (parser location) ()
  (let loop
    ((case 1)
     (events '()))
    (if (> case total)
